...
public class UIScreenManager : CachedMonoBehaviour 
{
	...
	/// </summary>
	public bool _mustRegisterForBackOperations = true;
	/// <summary>
	/// The must active recursively flag.
	/// </summary>
	public bool _mustActiveRecursively = true;
	/// <summary>
	/// The must survive scene change flag.(DontDestroyOnSceneChange)
	/// </summary>
	public bool _mustSurviveSceneChange = false;
	private bool _alreadyMarkedAsPerpetual = false;
	/// <summary>
	/// The unique screen identifier.
	/// </summary>
	public string 		_uniqueScreenId;
	/// <summary>
	/// The screen camera depth.
	/// </summary>
	public float _screenCameraDepth = 0.0f;
	/// <summary>
	/// The is pop up flag.
	/// </summary>
	public bool _isPopUp = false;
	/// <summary>
	...
	public void Init(UIScreenController.ScreenChangedEventHandler screenChangeCallback)
	{
		if(Application.isPlaying)
		{
			...
			{
				Debug.Log("Init UIScreenManager["+CachedGameObject.name+"] with Id["+_uniqueScreenId+"]");
			}

			if(_mustSurviveSceneChange && !_alreadyMarkedAsPerpetual)
			{
			...
			UpdateCameraDepth(_screenCameraDepth);
			if(_currentUIScreen == null)
			{
				if(_mustShowDebugInfo)
				{
	...
	public void Deactivate(UIScreenController.ScreenChangedEventHandler screenChangeCallback)
	{
		if(Application.isPlaying)
		{
			if(_mustShowDebugInfo)
			{
				Debug.Log("Deactivating UIScreenManager["+CachedGameObject.name+"] with Id["+_uniqueScreenId+"]");
			}
			if(_currentUIScreen != null)
			{
	...
	public void UpdateCameraDepth(float newDepth)
	{
		if(_mustShowDebugInfo)
		{
			Debug.Log("Updating screen["+_uniqueScreenId+"] depth to["+newDepth+"]");
		}
		if(newDepth >= -100 && newDepth <= 100)
		{
			_screenCameraDepth = newDepth;
		}
		if(_camera != null)
		{
			_camera.depth = _screenCameraDepth;
		}
	}
