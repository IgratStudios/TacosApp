...
public class UIScreenControllerDrawer : PropertyDrawer 
{
	...
	override public void OnGUI(Rect position, SerializedProperty property, GUIContent label) 
	{
		...
		SerializedProperty id = property.FindPropertyRelative("_uiUniqueId");
		string idPath = id.propertyPath;
		bool isPopUpController = idPath.Contains("PopUp");
		SerializedProperty destroyFlag = property.FindPropertyRelative("_doNotDestroyOnDeactivation");
		SerializedProperty startFlag = property.FindPropertyRelative("_isStartingScreen");

		SerializedProperty activeFlag = property.FindPropertyRelative("_isActive");
		SerializedProperty specialFlag = property.FindPropertyRelative("_isSpecialPopUp");

		SerializedProperty prefab = property.FindPropertyRelative("_uiScreenPrefab");
		SerializedProperty currentObject = property.FindPropertyRelative("_uiScreenObject");
//		SerializedProperty screenPosition = property.FindPropertyRelative("_uiScreenPosition");
		...
		if(property.isExpanded)
		{
			...
			if(manager != null)
			{
				if(manager is UIManager)
				{
					...
					if(uiManager._allScreenControllers.Count > 1)
					{
						...
						if(list.isExpanded)
						{
							...
							if((uiManager._allScreenControllers.Count - 1 - list.arraySize) > 0 )
							{
								...
								for (int i = 0; i < list.arraySize; i++) 
								{
									...
									if(newComplementIndex != currentComplementIndex)
									{
										...
										if(IsValidNewComplementAssigment(uiManager,list,newComplementIndex) &&
											uiManager._allScreenControllers[newComplementIndex]._uiUniqueId != id.stringValue)
										{
											list.GetArrayElementAtIndex(i).stringValue = uiManager._allScreenControllers[newComplementIndex]._uiUniqueId;
										}
										else
										{
							...
							{
								if(uiManager._allScreenControllers.Count > list.arraySize)
								{
				...
				else if(manager is PopUpsManager)
				{
					...
					if(!path.Contains("Background"))
					{
						...
						if(list.isExpanded)
						{
							if(popUpsManager._allPopUpBackgroundsControllers.Count > 0)
							{
								bool addNewBackground = false;
								EditorGUILayout.BeginHorizontal();
								EditorGUILayout.LabelField("["+list.arraySize+"] Backgrounds Related.");
								if((popUpsManager._allPopUpBackgroundsControllers.Count - list.arraySize) > 0 )
								{
								...
								if(!addNewBackground)
								{
									for (int i = 0; i < list.arraySize; i++) 
									{
										...
										if(newBkgIndex != currentBkgIndex)
										{
											...
											{
												list.GetArrayElementAtIndex(i).stringValue = popUpsManager._allPopUpBackgroundsControllers[newBkgIndex]._uiUniqueId;
											}
											else
											{
								...
								{
									if(popUpsManager._allPopUpBackgroundsControllers.Count > list.arraySize)
									{
										list.InsertArrayElementAtIndex(list.arraySize);
	...
	{
		for(int i = 0; i < manager._allPopUpBackgroundsControllers.Count; i++)
		{
			if(manager._allPopUpBackgroundsControllers[i]._uiUniqueId == currentBkgId)
			{
				return i;
			}
		}
	...
	{
		for(int i = 0; i < manager._allScreenControllers.Count; i++)
		{
			if(manager._allScreenControllers[i]._uiUniqueId == currentComplementId)
			{
				return i;
			}
		}
	...
	private string[] GetBackgroundsIds(PopUpsManager manager)
	{
		...
		for(int i = 0; i < manager._allPopUpBackgroundsControllers.Count; i++)
		{
			ids.Add( manager._allPopUpBackgroundsControllers[i]._uiUniqueId );
		}
		return ids.ToArray();
	}
	...
	private string[] GetScreenIds(UIManager manager)
	{
		...
		for(int i = 0; i < manager._allScreenControllers.Count; i++)
		{
			ids.Add( manager._allScreenControllers[i]._uiUniqueId );
		}
		return ids.ToArray();
	}
	...
	private string GetNotAssignedBkgId(PopUpsManager manager,SerializedProperty currentList)
	{
		...
		for(int i = 0; i < manager._allPopUpBackgroundsControllers.Count; i++)
		{
			selectedId = manager._allPopUpBackgroundsControllers[i]._uiUniqueId;
	...
	private string GetNotAssignedComplementId(UIManager manager,SerializedProperty currentList,string thisId)
	{
		...
		for(int i = 0; i < manager._allScreenControllers.Count; i++)
		{
			if(manager._allScreenControllers[i]._uiUniqueId != thisId)
			{
				selectedId = manager._allScreenControllers[i]._uiUniqueId;
				//check if it is already assigned
				bool assigned = false;
				for(int j = 0; j < currentList.arraySize; j++)
				{
	...
	private bool IsValidNewComplementAssigment(UIManager manager,SerializedProperty currentList,int newIndex)
	{
		...
		{
			if(currentList.GetArrayElementAtIndex(i).stringValue == manager._allScreenControllers[newIndex]._uiUniqueId)
			{
				return false;
			}
		}
	...
	private bool IsValidNewAssigment(PopUpsManager manager,SerializedProperty currentList,int newIndex)
	{
		for(int i = 0; i < currentList.arraySize; i++)
		{
			if(currentList.GetArrayElementAtIndex(i).stringValue == manager._allPopUpBackgroundsControllers[newIndex]._uiUniqueId)
			{
				return false;
			}
		}
